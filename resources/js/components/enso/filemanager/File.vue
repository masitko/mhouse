<template>
  <div class="box has-background-light raises-on-hover file-box has-padding-large">
    ...
    <h5 class="title is-5 has-text-centered">{{ file.name }}</h5>
    ...
    <div class="has-text-centered has-margin-top-medium">
      <div class="details">
        <a class="button is-naked" v-if="canAccess('core.files.link')" @click.stop="link">
          <span class="icon">
            <fa icon="link" />
          </span>
        </a>
        <a class="button is-naked" @click.stop="show" v-if="isViewable">
          <span class="icon">
            <fa icon="eye" />
          </span>
        </a>
        <a class="button is-naked" :href="downloadLink">
          <span class="icon">
            <fa icon="cloud-download-alt" />
          </span>
        </a>
        <popover placement="top" @confirm="$emit('delete')" v-if="file.isDeletable">
    ...
    <modal :show="temporaryLink !== ''" :link="temporaryLink" @close="temporaryLink = ''" />
  </div>
</template>
...
export default {
  ...
  methods: {
    link() {
      axios
        .get(route("core.files.link", this.file.id))
