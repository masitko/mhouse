<template>
  <div class="box has-background-light has-padding-medium raises-on-hover" v-if="profile">
    <h4 class="title is-4 has-text-centered has-margin-top-large">
      <span class="icon">
        <fa icon="user" size="xs"/>
      </span>
    ...
    <div class="columns">
        ...
        <div class="columns is-mobile">
          ...
          <div class="column">
            <div class="has-margin-top-small field controls">
              <a class="button is-fullwidth is-primary" v-if="isSelfVisiting" @click="updateAvatar">
                <span class="icon">
                  <fa icon="sync-alt"/>
                </span>
                <span>{{ __('Avatar') }}</span>
              ...
              >{{ __('Impersonate') }}</a>
              <a
                class="button is-fullwidth has-margin-top-small is-warning"
                @click="$router.push({
      ...
      <div class="column">
        <hr class="has-background-grey-lighter" v-if="isMobile">
        <div class="columns is-mobile is-multiline details">
          ...
          <div class="column is-two-thirds has-padding-small">
            <span v-if="profile.school" class="has-margin-left-medium">{{ profile.school.name }}</span>
          </div>
          <div class="column is-one-third has-text-right has-padding-small">
            <strong>{{ __('Role') }}:</strong>
          </div>
          <div class="column is-two-thirds has-padding-small">
            <span class="has-margin-left-medium">{{ profile.role.name }}</span>
          </div>
          <div class="column is-one-third has-text-right has-padding-small">
            <strong>{{ __('Email') }}:</strong>
          </div>
          <div class="column is-two-thirds has-padding-small">
            <span class="has-margin-left-medium">{{ profile.email }}</span>
          </div>
          <div class="column is-one-third has-text-right has-padding-small">
            <strong>{{ __('Phone') }}:</strong>
          </div>
          <div class="column is-two-thirds has-padding-small">
            <span class="has-margin-left-medium">{{ profile.phone }}</span>
          </div>
          <div class="column is-one-third has-text-right has-padding-small">
            <strong>{{ __('Birthday') }}:</strong>
          </div>
          <div class="column is-two-thirds has-padding-small">
            <span class="has-margin-left-medium">{{ dateFormat(profile.birthday) }}</span>
          </div>
          <div class="column is-one-third has-text-right has-padding-small">
...
export default {
  ...
  computed: {
    ...
    avatarId() {
      return this.isSelfVisiting ? this.user.avatar.id : this.profile.avatar.id;
    },
    avatarLink() {
  ...
  methods: {
    ...
    fetch() {
      let params =
        typeof this.$route.params.user !== "undefined"
          ? this.$route.params.user
          : this.$route.params.student;
      axios
        .get(route(this.$route.name, params))
        .then(response => (this.profile = response.data.user))
        .catch(error => this.handleError(error));
    },
    updateAvatar() {
      axios
        .patch(route("core.avatars.update", this.user.avatar.id))
    ...
    dateFormat(date) {
      return date ? format(date, this.meta.dateFormat) : null;
    }
  }
};
</script>
