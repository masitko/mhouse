<template>
    <div class="wrapper">
        <div v-if="controls"
             class="controls">
            <button v-if="canAccess('administration.schools.people.create')"
                    class="button"
                    @click="create()">
                <span v-if="!isMobile">
                    {{ __('Associate Person') }}
        ...
        <div class="columns is-multiline"
            ...
            <person-form v-if="path"
                ...
                @loaded="$refs.form.field('school_id').value = id"/>
        </div>
...
export default {
    ...
    methods: {
        fetch() {
            ...
            axios.get(route(
                'administration.schools.people.index', { school: this.id },
            )).then(({ data }) => {
                this.people = data;
                this.$emit('update');
                this.loading = false;
        ...
        create() {
            this.path = route(
                'administration.schools.people.create', { school: this.id },
            );
        },
        edit(person) {
            this.path = route(
                'administration.schools.people.edit', { person: person.id },
            );
        },
        destroy(person, index) {
            ...
            axios.delete(route(
                'administration.schools.people.destroy', { person: person.id },
            )).then(() => {
